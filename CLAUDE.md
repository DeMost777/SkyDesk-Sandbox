...
Каждый flow проходит две фазы по порядку. Текущая фаза flow — в шапке его flow doc (`Phase: …`).

1. **Coverage** — все экраны и состояния flow открываются. Пробелы в покрытии — в flow doc, раздел «Known gaps».
2. **Craft** — визуальная доработка. Начинается, когда «Known gaps» пуст, и flows при этом должны продолжать работать.

Почему такой порядок: полировать экран, до которого нельзя дойти или который потом поменяется из-за нового состояния, — потерянная работа.
...
npm run foundations  # пересобрать страницы Foundations из токенов (после любой правки токенов)
...
**Ветки и деплой** (2026-09-24). Основная ветка — `main`: в ней всегда последнее согласованное состояние. Каждая задача — в своей ветке, в `main` попадает через pull request. Vercel публикует `main` как production, каждая другая ветка получает свою preview-ссылку. Push или merge в `main` — релиз, только по явной просьбе. Перед началом задачи ветку создавать от свежего `main`, иначе работа снова разойдётся по веткам.
...
- **Foundations в Storybook генерируются из токенов** (`npm run foundations` → `src/foundations/generated/*.mdx`, 2026-09-25). Значения пишутся в MDX текстом, потому что Storybook MCP отдаёт агенту текст MDX: таблица, посчитанная в браузере, дошла бы до агента пустой. Образцы цветов — компонент `TokenSwatches`, он читает CSS-переменные сам и не раздувает текст для агента (страница Colors ≈1,4 тыс. токенов). `src/foundations/foundations.test.ts` падает, если страницы старше токенов или если размер раскладки `-[Npx]` из кода не записан в `src/foundations/layout.mdx`. Markdown-таблицы в MDX работают через `remark-gfm` (`.storybook/main.ts`); без него они выводятся строкой текста.
...
└── src/                    ← React-приложение
    ...
    ├── foundations/        ← Storybook Foundations: overview, layout (вручную), generated/ (из токенов)
...
10. **Storybook** — новый или изменённый компонент получает stories в том же изменении, рядом с компонентом (`*.stories.tsx`), и строку в `docs/components.md`. Stories — это и тесты: `npm test` должен проходить.
11. **`ROADMAP.md` — карта работ, не автопилот** (правило пользователя, 2026-09-24). Сессия не обязана брать задачу из списка: обычно задачу ставит пользователь в разговоре.
    ...
    - Задача закончена — перенести в «Сделано» в том же коммите, что и работа. Номер задачи — в начале сообщения коммита (`[1.5] …`): по нему коммит находится через `git log --grep`.
    - Всё непокрытое и замеченное по ходу (правило 8) — в «Потом».
    - Порядок в «Дальше» меняет только пользователь.
