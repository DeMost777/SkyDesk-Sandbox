...
- **Storybook — единственный каталог компонентов; отдельного `docs/components.md` нет** (решение пользователя, 2026-09-25). Что есть у компонента, смотреть в Storybook: страница Docs (autodocs включён для всех stories в `.storybook/preview.tsx`), для агента — MCP-сервер `@storybook/addon-mcp` (`http://localhost:6006/mcp`, пока идёт `npm run storybook`) или напрямую `*.stories.tsx`. Где что пишется:
  - назначение и Figma node — JSDoc над `const meta` в story: текст + тег `@figma <node id>`. Addon-mcp и страница Docs берут описание оттуда; JSDoc над самим компонентом манифест при этом игнорирует — не дублировать;
...
10. **Storybook** — новый или изменённый компонент получает stories в том же изменении, рядом с компонентом (`*.stories.tsx`), с JSDoc над `meta` (назначение, `@figma`, если node известен) — см. «Решения и gotchas». Stories — это и тесты: `npm test` должен проходить.
