...
Итерация 1: модель и mock-данные (2.2), Header (2.3), страница по адресу (2.4) готовы, пройдены в браузере (2.6, `docs/testing-plan.md` → «Последний прогон»). Нет перехода из PNR Search — отложен решением пользователя до утверждения поиска (2.5). В Booking ведут навигация sandbox и адрес.
